	          ...
	          <div class="card-tools">
	          	<table style="margin-left:60px;">
                	<tr>
                		<td style="padding-right:40px;">					
							<select class="form-control" name="com_state_code_val" id="comStateCode" onchange="searchList_go(1,'list');" style="border:none;">
			             		<option>가입 상태</option>
                		...
                		<td style="padding-right:40px;">						
							<select class="form-control" name="code_state" id="codeState" onchange="searchList_go(1,'list');" style="border:none;">
			             		<option>회원 상태</option>
						...
						<td>
							<div class="input-group" style="float:left;padding-right:40px;">
						...
						<td style="padding-right:40px;">
							<div class="input-group row" style="width:350px;">
								...
								<input  class="form-control col-sm-8" type="text" name="keyword" placeholder="검색어를 입력하세요." value="${param.keyword }" style="width:70px;outline: 0;border-width: 0 0 2px;"/>
						...
						<td>
						<button type="button" id="downloadBtn" class="btn btn-block btn-secondary" style="float:right;" onclick="fnExcelReport('hiddenTable','funseamding_일반회원리스트')">Excel</button>
						</td>
					  </tr>	
					</table>
				...
				<c:forEach items="${comMemberList }" var="comMember">
					<tr class="trForClick" style='font-size:0.85em;cursor:pointer;'>
						<td>${comMember.com_num }<input type="hidden" value="${comMember.mem_num }"></td>
						...
						<td><a href="#a" onclick="OpenWindow('detail?mem_num=${comMember.mem_num }','일반 회원 상세 보기','950','900')">${comMember.mem_name }</a></td>
						<td>${comMember.mem_email }</td>
						<td><img src="<%= request.getContextPath() %>/displayProfileImg?mem_email=${comMember.mem_email}&fileName=${comMember.mem_profile_img}" onerror="this.src='<%= request.getContextPath() %>/resources/images/profile-picture.png'" alt="user profile image" style="width:80px;height:80px;"></td>
						<td>${comMember.mem_phone }</td>
						<td>
							<fmt:formatDate value="${comMember.mem_regdate }" pattern="yyyy-MM-dd"/>
						</td>
						<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendSMSForm?mem_name=${comMember.mem_name }&mem_phone=${comMember.mem_phone }','문자 보내기','700','735')"><i class="far fa-comment-dots fa-1x"></i></button></td>
						<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendEmailForm?mem_name=${comMember.mem_name }&mem_email=${comMember.mem_email }','메일 보내기','700','800')"><i class="far fa-envelope fa-1x"></i></button></td>
					</tr>
				</c:forEach>
	          </tbody>
	         </table>
...
/* 	$(function(){
	...
	$(".trForClick").on('click', function(){
		var tr = $(this);
		var td = tr.children();
		var hiddenInput = td.eq(0).children();
		//alert(hiddenInput.val());
		var mem_num = hiddenInput.val();
		
		OpenWindow('<%= request.getContextPath() %>/admin/comMember/detail?mem_num=' + mem_num,'소상공인 회원 상세 보기',950,900);
	});	
