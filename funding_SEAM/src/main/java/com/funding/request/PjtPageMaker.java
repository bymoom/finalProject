...
public class PjtPageMaker {
	...
	private PjtSearchCriteria pjtCri; //현재 페이지 정보
	...
	public PjtSearchCriteria getPjtCri() {
		return pjtCri;
	}

	public void setPjtCri(PjtSearchCriteria pjtCri) {
		this.pjtCri = pjtCri;
	}
	...
	private void calcData(){
		...
		next = endPage * pjtCri.getPerPageNum() >= totalCount ? false : true;
	}
	
	
	
	public String makeQuery(){	
		String query="?page="+pjtCri.getPage()
				    +"&perPageNum="+pjtCri.getPerPageNum()
				    +"&categoryType="+pjtCri.getCategoryType()
				    +"&keyword="+pjtCri.getKeyword();
		return query;
	}
